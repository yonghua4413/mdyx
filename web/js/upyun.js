...
function uplocalFile(domid,token,$type){
    art.dialog.data('domid', domid);
    art.dialog.open('?g=User&m=Upyun&a=uploadlocalfile&token='+token+'&type='+$type,{lock:true,title:'上传文件',width:600,height:400,yesText:'关闭',background: '#000',opacity: 0.45});
}
...
function chooseFileTp(domid,type,callback,from){
	art.dialog.data('domid', domid);
	art.dialog.data('width', '80%');
	art.dialog.data('callback', callback);
	art.dialog.open('?g=User&m=Attachment&a=my&type='+type+'&from='+from,{lock:true,title:'选择文件',width:'70%',height:'80%',yesText:'关闭',background: '#000',opacity: 0.45,close: callback});
}
